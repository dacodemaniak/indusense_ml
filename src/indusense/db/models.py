...
class GoldMachineHourlyFeature(TimestampMixin, Base):
    ...
    # Short-lag delta features (gradient directionnel de la rampe pré-panne)
    temp_delta_1h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    temp_delta_3h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    pressure_delta_1h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    pressure_delta_3h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    rotation_delta_1h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    rotation_delta_3h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    voltage_delta_1h: Mapped[Decimal | None] = mapped_column(Numeric(10, 3))
    # Per-machine z-score (fittés sur stats train uniquement)
    temp_zscore_machine: Mapped[Decimal | None] = mapped_column(Numeric(8, 4))
    pressure_zscore_machine: Mapped[Decimal | None] = mapped_column(Numeric(8, 4))
